...
class LinearRegression(inputs: DenseMatrix[Double],
  ...
  /**
    ...
    * @param outputs The output training examples, by default they are the ones provided int the constructor
    * @param regularizationParam The regularization penalty weight, by default they are the ones provided int the constructor
    * @return
    */
  def train(inputs: DenseMatrix[Double] = x,
            outputs: DenseMatrix[Double] = outputs,
            regularizationParam: Double = 0) : DenseMatrix[Double] = {
    val l = inputs.cols
  ...
  def crossValidation(folds: Int,
    ...
    val xValError = partitions.foldRight(Vector.empty[Double]) {(c, acc) =>
      ...
      // append error to the accumulator so it can be average lator
      acc :+ error
    }
