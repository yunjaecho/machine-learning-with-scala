package com.yunjae.ml4s.unsupervised

import breeze.linalg.DenseVector

import scala.util.Random

case class Cluster(mean: DenseVector[Double], assignedDataPoint: Seq[DenseVector[Double]])
...
object KMeans {
  ...
  def assignDataPoints(clusterMeans: Seq[DenseVector[Double]],
    ...
    clusters.toSeq.map(c1 => Cluster(c1._1, c1._2.toSeq))
  }
  ...
  def cluster(dataset: Seq[DenseVector[Double]],
    ...
    var newClusterMeans = oldClusterMeans.map(mean => mean.map(_ + 0.1))

    var iterations = 0

    while (oldClusterMeans != newClusterMeans) {
      oldClusterMeans = newClusterMeans
      newClusterMeans = clusters.map(c => computeMean(c.assignedDataPoint))
